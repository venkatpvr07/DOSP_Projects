...
-import(hopmodule,[peer_wait/1,allHops/0]).
-export([start/2, begin_net/2, task_listen/2, actor/4,get_succ/4,closest_peer/3,next_peer/2,prev_peer/2]).

start(NodeCount, NumRequest) ->
    register(supervisor, spawn(main, begin_net, [NodeCount, NumRequest])).
...
add_chord(NetNodes, AllNodes, Bit_M, NetState) ->
    ...
    ProcessId = spawn(main, actor, [Peer, Bit_M, NetNodes, dict:new()]),
...
ping_and_destroy(NetNodes, NodeCount, NumRequest, Bit_M, NetState) ->
    register(taskcompletionmonitor, spawn(main, task_listen, [NodeCount * NumRequest, 0])),
